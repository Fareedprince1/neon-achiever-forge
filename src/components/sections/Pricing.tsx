...
type Plan = {
  name: string; price: number; popular?: boolean;
  features: { label: string; on: boolean }[];
};

const plans: Plan[] = [
  { name: "Basic", price: 999, features: [
  ...
  { name: "Pro", price: 1799, popular: true, features: [
  ...
  { name: "Elite", price: 2999, features: [
    ...
    <section id="pricing" className="py-24">
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-12">
          {plans.map((p) => {
            const monthly = yearly ? Math.round(p.price * 0.8) : p.price;
            return (
              <div key={p.name} className={cn(
                "rounded-3xl p-7 border transition-all relative",
                p.popular ? "bg-primary text-primary-foreground border-primary neon-glow scale-[1.02]" : "bg-card border-border hover:border-primary"
              )}>
                {p.popular && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-background text-primary border border-primary text-[10px] font-bold uppercase tracking-widest rounded-full px-3 py-1">
                    Most Popular
                  </span>
                )}
                <h3 className="text-display text-3xl uppercase">{p.name}</h3>
                <div className="mt-4">
                  <span className="text-display text-5xl">₹{monthly.toLocaleString("en-IN")}</span>
        ...
        <p className="text-center mt-8 text-muted-foreground">
          Not sure? <a href="#contact" className="neon-text hover:underline">Book a free consultation →</a>
        </p>
      </div>
    </section>
  );
